...
#define trigPin 2
#define echoPin 3
#define ENCODER_PIN A0 // Pin untuk encoder di pin analog A0
int IN1 = 8;
int IN2 = 4;
int IN3 = 5;
int IN4 = 7;
...
void readEncoder();
Task task3(10, TASK_FOREVER, &readEncoder); // Read encoder data
...
void measureDistance() {
  ...
  // Send the distance and pulseCount to the serial port
...
void readEncoder() {
  // Baca nilai analog dari pin encoder dan konversi menjadi digital
  int encoderValue = analogRead(ENCODER_PIN);
  if (encoderValue > 512) { // Threshold untuk menentukan logika HIGH
    pulseCount++;
  }
}
